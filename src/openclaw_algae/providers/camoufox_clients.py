...
from openclaw_algae.browser.common import (
    build_cookie_string,
...
from openclaw_algae.providers.glm import GLMApiClient, GLMIntlApiClient
from openclaw_algae.providers.glm import _extract_glm_text_candidates, _glm_meta_data_for_model
...
from openclaw_algae.storage.provider_sessions import load_provider_session, save_provider_session
...
class CamoufoxProviderClient:
    ...
    def _chat_kimi(self, *, message: str, model: str) -> str:
        def action(context, page):
            ...
            if not kimi_auth:
                raise RuntimeError("Kimi auth cookie not found. Re-run `openclaw-algae login kimi`.")
            scenario = (
                "SCENARIO_SEARCH"
                if "search" in model
            ...
            if refresh_token and (not access_token or _is_jwt_expired(access_token)):
                ...
                if refreshed:
                    ...
                    raise RuntimeError(
                        ...
                        "Run `openclaw-algae login glm cn` to re-authenticate."
                    )
...
def _iter_stream_with_startup_timeout(
    ...
    *,
    ...
    threading.Thread(
        target=worker,
        name=f"openclaw-stream-startup-{description.lower().replace(' ', '-')}",
        daemon=True,
    ).start()
    ...
    *,
    ...
    if not xsrf_token or not ut:
        raise RuntimeError(
            ...
            "Run `openclaw-algae login qwen-cn` again."
        )
...
def _stream_doubao_browser_completion(
    ...
    *,
    ...
    stream_id = f"__openclaw_algae_doubao_stream_{uuid4().hex}"
    started = page.evaluate(
        ...
        ({ streamId, body, params }) => {
          ...
          state.abort = (reason) => {
            try {
              controller.abort(reason || "openclaw-cleanup");
            } catch (_error) {}
          };
...
def _stream_glm_cn_browser_completion(
    ...
    *,
    ...
    assistant_id = _GLM_ASSISTANT_ID_MAP.get(model, "65940acff94777010aa6b796")
    sign = _generate_glm_sign()
    stream_id = f"__openclaw_algae_glm_cn_stream_{uuid4().hex}"
    started = page.evaluate(
        """
...
def _stream_qwen_intl_browser_completion(
    ...
    *,
    ...
    stream_id = f"__openclaw_algae_qwen_intl_stream_{uuid4().hex}"
    started = page.evaluate(
        """
