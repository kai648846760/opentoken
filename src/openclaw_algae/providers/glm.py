...
class GLMWebAdapter(ProviderAdapter):
    ...
    def chat(
        ...
        if credentials is None:
            raise RuntimeError("Missing GLM credentials. Run `openclaw-algae login glm cn` first.")
...
class GLMIntlWebAdapter(ProviderAdapter):
    ...
    def chat(
        ...
        if credentials is None:
            raise RuntimeError("Missing GLM Intl credentials. Run `openclaw-algae login glm international` first.")
        client = self._client_factory(credentials)
        content = client.chat_completion(
            message=build_role_prompt(request),
            model=request.model.rsplit("/", 1)[-1],
